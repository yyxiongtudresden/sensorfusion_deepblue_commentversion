...
int main(int argc, char *argv[]) {
    ...
    ros::NodeHandle nh;
    // 执行类的构建函数时候,自动调用private的msg_callback函数.也就是接收和处理信息的地方,接收从ros中输出的数据.
        ...
        if (!transform_received) {
            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0) {
                ...
                if (d_time < -0.05) {
                    ...
                    // block矩阵块操作,把右边得到的旋转矩阵R赋值到左边odometry_matrix 从 00开始的 3*3矩阵 <3,3>(0,0),构建成一个4*4的rigid transformation 
